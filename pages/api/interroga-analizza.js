...
export default async function handler(req, res) {
  ...
  const { accessToken, photo, photos, argomento, materia, classe, sesso, nome } = req.body;
...
Livello studente: ${adattivita}`, cache_control: { type: "ephemeral" } }],
      ...
      // Controlli sicurezza in parallelo per tutte le foto
...
PERSONALE: selfie, bambini fotografati, persone, foto private.`, cache_control: { type: "ephemeral" } }],
      ...
      // Unica chiamata Claude con tutte le immagini
...
Livello studente: ${adattivita}`, cache_control: { type: "ephemeral" } }],
        messages: [{ role: "user", content: [
          ...imageContent,
          { type: "text", text: `Analizza ${photosList.length > 1 ? `queste ${photosList.length} foto di appunti` : "questi appunti"} e prepara l'interrogazione.` }
        ]}],
      });
